...
const normalizeStatus = (
  status: string,
): 'Abierto' | 'Contratado' | 'Cerrado' | 'Borrador' => {
  const loweredStatus = status.toLowerCase();

  if (loweredStatus === 'open') {
    return 'Abierto';
  }

  if (loweredStatus === 'hired') {
    return 'Contratado';
  }

  if (loweredStatus === 'closed') {
    return 'Cerrado';
  }
...
export const getPositionsService = async () => {
  ...
  return positions.map((position) => ({
    ...
    manager: position.contactInfo ?? 'Sin asignar',
    deadline: position.applicationDeadline
      ? position.applicationDeadline.toISOString().slice(0, 10)
      : '',
